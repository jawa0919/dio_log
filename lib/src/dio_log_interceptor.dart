...
class DioLogInterceptor implements Interceptor {
  ...
  void saveResponse(Response response) {
    ...
    if (enablePrintLog) {
      ...
      debugPrint('request: url:${logNp.reqOptions?.url}');
      debugPrint('request: method:${logNp.reqOptions?.method}');
      debugPrint('request: params:${logNp.reqOptions?.params}');
      debugPrint('request: data:${logNp.reqOptions?.data}');
      debugPrint(
          'request: duration:${getTimeStr1(logNp.reqOptions!.requestTime!)}');
      debugPrint('response: ${toJson(logNp.resOptions?.data)}');
    }
  }
}
